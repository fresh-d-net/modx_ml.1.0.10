...
$lang['url'] = 'Файл либо ссылка:';
$lang['link'] = 'Link (to big image):';
$lang['title'] = 'Название:';
...
if ($e->name == 'OnDocFormRender' ) {
	...
	$output = isset($matches[0]) ? $matches[0] : '' ;
	$output .= <<< OUT
		<!-- MultiTv -->
			<style type="text/css">
				.fotoitem {border:1px solid #e3e3e3; margin:0 0 5px; padding:2px 5px 5px 5px; overflow:hidden; white-space:nowrap; zoom:1}
				.fotoitem span {display:inline-block; padding-top:3px;}
				.fotoitem input {line-height:1.1; vertical-align:middle;}
				.fotoimg {float:right; padding-top:3px;}
			</style>
			<script type="text/javascript">
				var MultiTv = new Class({
					...
					addItem: function(values,elem){
						...
						if (!values) values=['','',''];
						var fileUrl = new Element('input',{'type':'text','name':'foto_'+this.name+'_'+f,'id':'foto_'+this.name+'_'+f,'class':'imageField','value':values[0],'events':{
						...
						var bInsert = new Element('input',{'type':'button','value':'{$lang['insert']}','events':{
							'click':function(){this.lastfoto='foto_'+this.name+'_'+f; BrowseFileServer('foto_'+this.name+'_'+f)}.bind(this)
						}});

						//create linkUrl field
						...
						var bInsertLink = new Element('input',{'type':'button','value':'{$lang['insert']}','events':{
							'click':function(){this.lastfoto='link_'+this.name+'_'+f; BrowseFileServer('link_'+this.name+'_'+f)}.bind(this)
						}});

						var fileName = new Element('input',{'type':'text','class':'imageField','value':values[1],'events':{
							'keyup':function(){this.setEditor();documentDirty=true;}.bind(this)
						}});
						...
						//insert elements in to container
						rowDiv.adopt(
							this.sp('{$lang['url']}'), this.br(),
							fileUrl, bInsert, this.br()
							//this.sp('{$lang['link']}'), this.br(),
							//linkURL, bInsertLink, this.br()
						);
						rowDiv.adopt(this.sp('{$lang['title']}'),this.br(),fileName, this.br(), bAdd);
				...
				window.addEvent('domready', function(){
					...
					for (var i=0;i<tvIds.length;i++){
						var fid = 'tv'+ tvIds[i];
						if($(fid)!=null) {var modxMultiTv=new MultiTv(fid);}
					}
				});
			</script>
		<!-- /MultiTv -->
...
foreach ($tvIds as $tvid) {
	$modx->logEvent(1, 3, 'Debug-><pre>' . print_r($th_width, true) . '</pre>', 'MultiTv. Log.');

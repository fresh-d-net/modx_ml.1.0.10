...
<?php
                $sql = 'SELECT t.templatename, t.description, t.id, c.category FROM '.$tbl_site_templates.' t LEFT JOIN '.$tbl_categories.' c ON t.category = c.id ORDER BY c.category, t.templatename ASC';
                $rs = mysql_query($sql);
                ...
                while ($row = mysql_fetch_assoc($rs)) {
                    ...
                    echo "\t\t\t\t\t".'<option value="'.$row['id'].'"'.$selectedtext.' title="'. $row['templatename'] .'">'. "{$row['description']} - {$row['templatename']}"."</option>\n";
